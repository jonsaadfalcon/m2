...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def from_composer(cls,
        ...
        if expand_positional_embeddings:
            ...
            #pdb.set_trace()
            import numpy as np
            position_embeddings_randomized_1 = torch.from_numpy(np.random.uniform(-1, 1, size=(original_embedding.shape[0], original_embedding.shape[1])))
            position_embeddings_randomized_2 = torch.from_numpy(np.random.uniform(-1, 1, size=(original_embedding.shape[0], original_embedding.shape[1])))
            position_embeddings_randomized_3 = torch.from_numpy(np.random.uniform(-1, 1, size=(original_embedding.shape[0], original_embedding.shape[1])))
